...
def chat_endpoint(request: RequestState):
    ...
    if request.model_name not in settings.ALLOWED_MODEL_NAMES:
        logger.warning(f"Model name {request.model_name} is not allowed.")
        raise HTTPException(status_code=400, detail=f"Model name {request.model_name} is not allowed.")
